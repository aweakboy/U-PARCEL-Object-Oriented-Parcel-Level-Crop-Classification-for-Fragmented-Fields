...
from src.backbones.ltae_stsa import ParallelSTSA_LTAE2d as LTAE2d
from src.backbones.attn_ext import STSA_TimeEncoder, CrossAttention, PA
...
class UTAE(nn.Module):
    def __init__(
        ...
        # self.temporal_encoder = STSA_TimeEncoder(
        #     in_channels=encoder_widths[-1],
        #     n_head=n_head,
        #     mlp_ratio=4.,
        #     drop=0.1
        # )
        # # 额外模块
        # self.cross_attn = CrossAttention(dim=encoder_widths[-1])
        # self.pa_head = PA(dim=encoder_widths[-1])
    ...
    def forward(self, input, batch_positions=None, return_att=False):
        ...
        if att is None:
            # 临时把 aggregator 切到 mean
            skip = feature_maps[-2].mean(dim=1)  # [B,C,H,W]
        else:
        ...
        for i in range(self.n_stages - 1):
            if att is None:
                skip = feature_maps[-(i + 2)].mean(dim=1)
            else:
                skip = self.temporal_aggregator(
        ...
        if self.encoder:
            ...
            # ① 先做随机失活（只在 model.train() 时生效）
            out = self.dropout(out)  # ← 新增这一行

            # ② 再接 1×1 输出卷积得到每类的 logits
