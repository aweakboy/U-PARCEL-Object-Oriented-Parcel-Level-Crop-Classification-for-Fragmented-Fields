...
class STSA_Encoder(nn.Module):
    def __init__(self, emb_dim, emb_dim_temporal, mlp_dim, num_heads, dropout_rate, attn_dropout_rate,seq_len=11):
        ...
        skip_dim = int(emb_dim_temporal * seq_len / emb_dim)
...
class STMA(nn.Module):
    def __init__(self, input_band, emb_dim, mlp_dim, num_heads, num_classes, attn_dropout_rate, dropout_rate,seq_len=9):
        super(STMA, self).__init__()

        # previous convolution
        self.seq_len=seq_len
        self.fe = Feat_extractor(input_band=2)
        self.conv1 = nn.Conv2d(seq_len*2, emb_dim, kernel_size=1, stride=1)
        self.conv2 = nn.Conv2d(seq_len*4, emb_dim, kernel_size=1, stride=1)
        self.avg1 = nn.AvgPool2d(2, 2)
        self.avg2 = nn.AvgPool2d(1, 1)

        self.with_pos = True
        if self.with_pos:
            self.pos = PA(dim=8*self.seq_len)
            self.pos_64 = PA(dim=2*self.seq_len)
            self.pos_128 = PA(dim=4*self.seq_len)

        # each transformer for each scale feature
        self.transformer1 = STSA_Encoder(
            emb_dim = 2*self.seq_len,
    ...
    def forward(self, x,batch_positions=None):
        ...
        x2_2 = x2.view(x2_b, x2_t, x2_c, x2_h, x2_w).flatten(2) #[b, 19, 1024*2]
        ...
        x3_2 = x3.view(x3_b, x3_t, x3_c, x3_h, x3_w).flatten(2) #[b, 19, 256*4]
        ...
        x_2 = x.view(x_b, x_t, x_c, x_h, x_w).flatten(2) #[b, 19, 256*8]
